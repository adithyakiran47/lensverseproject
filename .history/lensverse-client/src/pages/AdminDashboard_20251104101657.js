/ AdminDashboard.js - improved admin booking list with search/filter and responsive UI
    ...
    <>
      ...
      <div className="container bg-black text-white py-5 min-vh-100">
            ...
            <tbody>
              {filtered.map(b => (
                <tr key={b._id}>
                  ...
                  <td>
                    <span className={`badge bg-${STATUS_BADGE[(b.status || 'pending')] || 'secondary'}`}>
                      {b.status || 'pending'}
                    </span>
                  </td>
                  <td>
                    <div className="d-flex gap-2 flex-wrap">
                      <button className="btn btn-sm btn-success" onClick={() => updateBookingStatus(b._id, 'confirmed')}>
                        Confirm
                      </button>
                      <button className="btn btn-sm btn-warning" onClick={() => updateBookingStatus(b._id, 'cancelled')}>
                        Cancel
                      </button>
                      <button className="btn btn-sm btn-outline-light" onClick={() => { navigator.clipboard?.writeText(JSON.stringify(b)); showMessage('success','Copied booking to clipboard'); }}>
                        Copy
                      </button>
                      <button className="btn btn-sm btn-danger" onClick={() => deleteBooking(b._id)}>
                        Delete
                      </button>
                    </div>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan="6" className="text-muted text-center">No bookings found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="d-md-none">
          ...
          {filtered.length === 0 && (
            <div className="text-muted text-center py-4">No bookings found.</div>
          )}
        </div>
      </div>
    </>
